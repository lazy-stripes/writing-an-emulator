...
// The user chooses the FIFO's size at creation time, which is the whole point
// of this example.
type FIFO struct {
	fifo []interface{} // Array of values in the FIFO.
...
// NewFIFO returns a FIFO with the desired size.
func NewFIFO(size int) *FIFO {
	return &FIFO{fifo: make([]interface{}, size)}
}
...
func NewSDL() *SDL {
	...
	window, err := sdl.CreateWindow("FIFO overrun",
...
func run() {
	...
	mainthread.Call(func() {
		...
		// This is where the distracted, premature optimizer might accidentally
		// instantiate a micro-operations FIFO and either mistype or wrongly
		// think that 3 operations is /quite enough thank you/...
		ppu.Fetcher.mmu = &mmu
		cpu := &CPU{mmu: mmu, ops: *NewFIFO(3)} // FIXME: we need at least 5 ops
